...
# ─── CONFIGURATION ───────────────────────────────────────────────────────────────
# Replace this with the actual Hugging Face repo ID for Qwen2.5‐Vision 7B
...
class VLMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Load the ground‐truth JSON report
...
if __name__ == "__main__":
    ...
    # 2) Load image processor and tokenizer
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        ...
        use_fast=False  # requires sentencepiece
    )

    # 3) Load config and set up 4-bit quantization
    ...
    # 4) Load Qwen2.5 vision+LLM model with QLoRA
    ...
    # 9) Evaluate all 20 000 images, save JSON
    ...
    for img_path, _ in pairs:
        ...
        out_file = os.path.join(EVAL_OUT_DIR, image_id + ".json")
